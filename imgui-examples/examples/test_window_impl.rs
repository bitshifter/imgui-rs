...
fn show_test_window(ui: &Ui, state: &mut State, opened: &mut bool) {
    ...
    window.build(|| {
        ...
        if ui.collapsing_header("Help").build() {
          ui.text_wrapped("This window is being created by the show_test_window() \
        ...
        if ui.collapsing_header("Widgets").build() {
            ...
            ui.tree_node("Multi-line text").build(|| {
            ...
            ui.tree_node("Word Wrapping").build(|| {
              ui.text_wrapped("This text should automatically wrap on the edge of \
            ...
            ui.tree_node("UTF-8 Text").build(|| {
              ui.text_wrapped("CJK text will only appear if the font was loaded \
            ...
            ui.drag_int("drag int", &mut state.i0).build();
            ui.input_float("input float", &mut state.f0)
                .step(0.01)
                .step_fast(1.0)
                .build();
            ui.drag_float("drag float", &mut state.f0)
                .speed(0.001)
                .min(-1.0)
                .max(1.0)
                .build();
...
CTRL+click on individual component to input value.\n",
                ...
                let mut b = ui
                    .color_picker("MyColor##4", &mut s.color)
                    .flags(misc_flags)
                    .alpha(s.alpha)
        ...
        if ui
            .collapsing_header("Popups & Modal windows")
            .build()
        {
            ui.tree_node("Popups").build(|| {
              ui.text_wrapped("When a popup is active, it inhibits interacting \
...
fn show_example_app_custom_rendering(ui: &Ui, state: &mut CustomRenderingState, opened: &mut bool) {
    ui.window("Example: Custom rendering")
        ...
        .build(|| {
            ...
            ui.color_edit("Color", &mut state.col).build();
            ...
            if ui.button("Clear", (0.0, 0.0)) {
                state.points.clear();
            }
            if state.points.len() >= 2 {
                ui.same_line(0.0);
                if ui.button("Undo", (0.0, 0.0)) {
                    state.points.pop();
                    state.points.pop();
                }
            }
